...
import '../App.css';
import {login,save1,save2,save3,save4,save5,save6,save9} from '../req/api.js';
...
const  changee3 = (e)=>{  
  ...
  const fetchApi1 = useCallback(async () => {
    const p0 ={
      ...
      // otherInformation:schoolTestimonialPerson,
      // nomineePosition:schoolTestimonialPosition,

      // whanauEndorsement:whanauTestimonialPerson,
      // endorserRelationship:whanauTestimonialRelationship,
    ...
    let param = new URLSearchParams();
      ...
      // studentName:studentName,
    if(nominatingSchool){
      param.append("nominatingSchool",nominatingSchool);
    }
    if(studentName){
      param.append("studentName",studentName);
    }
    if(level){
      param.append("level",level);
    }
    if(schoolYear){
      param.append("year",schoolYear);
    }
    if(communityInvolvement){
      param.append("community",communityInvolvement);
    }
    if(Leadership){
      param.append("leadership",Leadership);
    }
    if(Academic){
      param.append("academic",Academic);
    }
    if(Sporting){
      param.append("sporting",Sporting);
    }
    if(schoolTestimonialPerson){
      param.append("otherInformation",schoolTestimonialPerson);
    }
    if(schoolTestimonialPosition){
      param.append("nomineePosition",schoolTestimonialPosition);
    }
   if(whanauTestimonialPerson){
    param.append("whanauEndorsement",whanauTestimonialPerson);
   }
   if(whanauTestimonialRelationship){
    param.append("endorserRelationship",whanauTestimonialRelationship);
   }
   if(mediaConsentMinor){
    param.append("mediaConsentMinor",mediaConsentMinor);
   }
   if(mediaContentDiscretion){
    param.append("mediaContentDiscretion",mediaContentDiscretion);
   }
   if(mediaConsentUpdate){
    param.append("mediaConsentUpdate",mediaConsentUpdate);
   }
   if(providerName){
    param.append("providerName",providerName);
   }
   if(providerPhone){
    param.append("providerPhone",providerPhone);
   }
   if(providerEmail){
    param.append("providerEmail",providerEmail);
   }

 
    const res0 = await save9(param);
  ...
  <div className="App">
    ...
    <select class="form-select" aria-label="Default select example" onChange = {chang3}>
      ...
      <option value="l1">Teina/Junior</option>
      <option value="l2">Tuakana/Senior</option>
    </select>
    </div>
    <div className="col">
    ...
    <select class="form-select" aria-label="Default select example"  onChange = {chang6}>
      ...
      <option value="y 1">Year 9</option>
      <option value="y 2">Year 10</option>
      <option value="y 3">Year 11</option>
    </select>
    </div>
    </div>
    <div className="row">
